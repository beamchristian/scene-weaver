// prisma/seed.ts
import { PrismaClient } from "../app/generated/prisma/index.js"; // <--- ADD /index.js
...
async function main() {
  ...
  const createdNodes: { [title: string]: string } = {};
  ...
  for (const nodeData of nodesData) {
    ...
    console.log(
      `Upserted NarrativeNode: ${upsertedNode.title} (ID: ${upsertedNode.id})`
    );
  }
  ...
  for (const choiceData of choicesData) {
    ...
    console.log(
      `Created Choice: "${choiceData.text}" from node ${choiceData.sourceNodeId}`
    );
  }
